...
namespace WinNodeEditorDemo.Core.Extensions
{
    public class ToolStripRendererEx : ToolStripRenderer
    {
        private readonly SolidBrush m_brush = new(Color.FromArgb(255, 52, 86, 141));
        private readonly StringFormat m_sf = new();
        ...
        protected override void InitializeItem(ToolStripItem item)
        {
            ...
            SetAutoSizeAndSizeRecursively(item);
        }
        protected void SetAutoSizeAndSizeRecursively(ToolStripItem item)
        {
            item.AutoSize = false;
            ...
            if (item is ToolStripDropDownItem dropDownItem)
            {
                ...
                {
                    SetAutoSizeAndSizeRecursively(childItem);
                }
            }
        }
        protected override void OnRenderToolStripBackground(ToolStripRenderEventArgs e)
        {
            using (SolidBrush sb = new(Color.FromArgb(34, 34, 34)))
            {
        ...
        {
            Point ptEnd = new(e.Item.ContentRectangle.X + (e.Item.Width / 2), e.Item.ContentRectangle.Y);
            using (LinearGradientBrush lgb = new(e.Item.ContentRectangle.Location, ptEnd, Color.Transparent, Color.Gray))
            {
